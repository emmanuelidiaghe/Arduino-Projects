...
void loop(void)
{
   ...
   dmd.drawMarquee("WELCOME TO EMMY_BEST RESTAURANT & BAR      ",40,(32*DISPLAYS_ACROSS)-1,0);
   long start=millis();
   long timer=start;
   boolean ret=false;
   while(!ret){
   ...
   dmd.drawMarquee("PLEASE RELAX AND HAVE A GOOD TIME WITH US...     ",50,(32*DISPLAYS_ACROSS)-1,0);
   long start1=millis();
   long timer1=start1;
   boolean ret1=false;
   while(!ret1){
