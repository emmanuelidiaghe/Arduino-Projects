 /* Virtuino ESP8266 web server Library ver 1.50
      ...
      if (c=='\n') {                                          // check command line to line
          ...
          if (pos>=0){                                        // check for GET command from Virtuino app
              ...
              if (pos!=-1){                                   // We have a GET message
                  String response = checkVirtuinoCommand(espBuffer.substring(pos+5));
                  boolean b=wifiSendData(connectionId,response);
                  clearESP_buffer(50);
             } 
          } 
...
//======================================================================================== esp8266_reset
//========================================================================================
boolean VirtuinoEsp8266_WebServer::esp8266_reset(void){
    unsigned long start;
    clearESP_buffer(100);    // clear esp buffer
    espSerial->println("AT+RST");
    return (responseString("OK",3000));
}

//======================================================================================== esp8266_mode
//========================================================================================
boolean VirtuinoEsp8266_WebServer::esp8266_mode(int mode){
    clearESP_buffer(100);    // clear esp buffer
    espSerial->print("AT+CWMODE=");
    espSerial->println(mode);
    return responseString("OK", "no change",3000);
}
//======================================================================================== esp8266_info
//========================================================================================
boolean VirtuinoEsp8266_WebServer::esp8266_info(){
    clearESP_buffer(100);    // clear esp buffer
...
//======================================================================================== esp8266_baud
//========================================================================================
boolean VirtuinoEsp8266_WebServer::esp8266_baud(long baud){
    clearESP_buffer(100);    // clear esp buffer
...
//======================================================================================== esp8266_connect
//========================================================================================
boolean VirtuinoEsp8266_WebServer::esp8266_connect(String ssid, String pwd){
    clearESP_buffer(100);    // clear esp buffer
    ...
    espSerial->print(pwd);
    espSerial->println("\"");
    return responseString("OK",10000);
}
//======================================================================================== esp8266_printIP
//======================================================================================== 
boolean VirtuinoEsp8266_WebServer::esp8266_printIP(){
    clearESP_buffer(100);    // clear esp buffer
...
//======================================================================================== esp8266_multiple
//========================================================================================
boolean VirtuinoEsp8266_WebServer::esp8266_multiple(int mode){
    clearESP_buffer(100);    // clear esp buffer
...
//======================================================================================== esp8266_serverPort
//========================================================================================
boolean VirtuinoEsp8266_WebServer::esp8266_serverPort(int port){
        clearESP_buffer(100);    // clear esp buffer
        espSerial->print("AT+CIPSERVER=1,");
        espSerial->println(port);
        return responseString("OK", "no change",3000);
}
//======================================================================================== esp8266_softAccessPoint
//========================================================================================
boolean VirtuinoEsp8266_WebServer::esp8266_softAccessPoint(String ssid, String pw){
        clearESP_buffer(100);    // clear esp buffer
...
boolean VirtuinoEsp8266_WebServer::wifiInternetConnect(String wifiNetworkName,String wifiNetworkPassword, int port=80){
    if (DEBUG) Serial.println(wf_END_LINE+wf_serverInitString);
    //esp8266_reset(); 
    //delay(2000);
    ...
    if (esp8266_connect(wifiNetworkName,wifiNetworkPassword)){
        ...
        if (DEBUG) Serial.println(wf_END_LINE+wf_serverReadyString);
        return true;
    }
    else {
      if (DEBUG) Serial.println(wf_END_LINE+wf_serverNotConnectedString);
      return false;
    }
}
...
//======================================================================================
String  VirtuinoEsp8266_WebServer::checkVirtuinoCommand(String command){
  ...
  if (pos>5){                                          // check the size of command
      ...
      if (DEBUG) Serial.println(wf_END_LINE+"cmd:"+ command1);

      //------------------  get command password
       String commandPassword="";
       int k=command1.indexOf(wf_COMMAND_START_CHAR);
       if (k>0) commandPassword = command1.substring(0,k);
      if (DEBUG) Serial.println(wf_END_LINE+"ps:"+commandPassword ); 
       
      if ((password.length()==0) || (commandPassword.equals(password))) {                       // check if password correct
           while (command1.indexOf(wf_COMMAND_START_CHAR)>=0){
              int cStart=command1.indexOf(wf_COMMAND_START_CHAR);
              int cEnd=command1.indexOf(wf_COMMAND_END_CHAR);
              if (cEnd-cStart>5){
              String oneCommand = command1.substring(cStart+1,cEnd);                               // get one command
                char commandType = getCommandType(oneCommand);
                  if (commandType!='E') {
                     ...
                     } else  returnedString +=getErrorCommand(wf_ERROR_PIN);  // response  error pin number   !E00=1$   
                  } else returnedString +=getErrorCommand(wf_ERROR_TYPE);  // response  error type   !E00=3$   
          
              } else{
                returnedString +=getErrorCommand(wf_ERROR_SIZE);  // response  error size of command   !E00=4$   
              }
              command1=command1.substring(cEnd+1);
           }  // while
      } else returnedString=getErrorCommand(wf_ERROR_PASSWORD);     // the password is not correct
  }
  else  returnedString=getErrorCommand(wf_ERROR_SIZE);         // the size of command is not correct
...
//======================================================================================== responseString
//========================================================================================
boolean VirtuinoEsp8266_WebServer::responseString(String target1,  int timeout){
    String data;
    char a;
...
//======================================================================================== responseString
//========================================================================================
boolean VirtuinoEsp8266_WebServer::responseString(String target1, String target2, int timeout)
{
    String data;
    char a;
    boolean rValue=false;
...
boolean VirtuinoEsp8266_WebServer::wifiSendData(int connectionId, String content){
 ...
 // HTTP Header
  ...
  httpResponse += wf_END_LINE + "Content-Type: text/html; charset=UTF-8"+wf_END_LINE;
...
String VirtuinoEsp8266_WebServer::getErrorCommand(byte code){
  return wf_COMMAND_START_CHAR+ wf_COMMAND_ERROR+String(code)+wf_COMMAND_END_CHAR;
}

//======================================================================================== executeCommandFromApp
//========================================================================================
  String VirtuinoEsp8266_WebServer::executeReceivedCommand(char activeCommandType, int activeCommandPin ,float activeCommandValue, boolean returnInfo){
    //------------------------------------
      ...
      case 'Q': 
       ...
       case 'O': 
        ...
        //         connectedStatus=true;
                 response ="!C"+pinString+"="+wf_firmwareCode+"$";
               }
              else if (activeCommandValue==0) {
            //    connectedStatus=false;
                response =wf_COMMAND_START_CHAR+"C"+pinString+"=0"+wf_COMMAND_END_CHAR;
               }
              else      response =getErrorCommand(wf_ERROR_VALUE);;
              
            } else   response =getErrorCommand(wf_ERROR_COMMAND);        
          break;  
           }
